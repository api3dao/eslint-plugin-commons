...
6. Rename `@shopify/prefer-early-return` comments to `unicorn/prefer-early-return`. The `@shopify/eslint-plugin`
   dependency was dropped, `prefer-early-return` now comes from `eslint-plugin-unicorn`, and
   `@shopify/prefer-module-scope-constants` is enforced by `@typescript-eslint/naming-convention`.
7. Run `eslint --fix` and then clean up whatever is left. Expect some stale `eslint-disable` directives to be reported,
