...
module.exports = [
  // Lints package.json itself. It carries its own "files" and JSON language, so it is not scoped to the source globs.
  packageJson.configs.recommended,
  {
    files: ['**/package.json'],
    rules: {
      // Pinning an exact version is a reasonable choice for some dependencies.
      'package-json/dependency-version-range': 'off',
      // We deliberately ship CommonJS across the org, so this rule is inverted for us.
      'package-json/prefer-type-module': 'off',
      // Every hit is a path inside a container, e.g. "--volume $(pwd)/config:/app/config", which the rule cannot tell
      // apart from a host path.
